...
func (e *engine) ExportCompiledModule(module *wasm.Module) (*wasm.CompiledModuleNativeCode, error) {
	...
	if !ok {
		...
		if !ok {
			return nil, fmt.Errorf("compiled module not found")
		}
	}

	offsets := make([]uint64, len(cm.functionOffsets))
	for i, offset := range cm.functionOffsets {
		offsets[i] = uint64(offset)
	}

	return &wasm.CompiledModuleNativeCode{
		GOOS:            runtime.GOOS,
		GOARCH:          runtime.GOARCH,
		Engine:          "wazevo",
		Code:            slices.Clone(cm.executable),
